...
class Slider {
  constructor(el) {
    ...
    this.nImages = this.contentEl.getAttribute("data-n-images");
  ...
  runAnimation() {
    ...
    this.contentEl.style.setProperty(
      ...
      `
        translateZ(${this.zDistance})
        rotateX(${maxY * yCoeff}deg) rotateY(${maxX * xCoeff}deg)
      `
    );
  ...
  next() {
    if (this.inTransit || +this.nImages <= 1) return;
...
const sliderEl = document.getElementById("slider-presentacion");
